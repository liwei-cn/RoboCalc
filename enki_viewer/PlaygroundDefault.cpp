...
class EnkiPlayground : public ViewerWidget          //comments
{
protected:
	#ifdef USE_SDL
...
public:
		...
		subjectiveView(false)
	{
		...
		#ifdef PROBLEM_GENERIC_TOY
		{
			...
			o->setCustomHull(hull, -1);           // a high light blue cylinder
			o->setColor(Color(0.4,0.6,0.8));
			o->pos = Point(10, 10);  
			world->addObject(o);
		}
		
		for (int i = 0; i < 20; i++)   //20 red cylinders
		{
			PhysicalObject* o = new PhysicalObject;
			o->pos = Point(UniformRand(20, 100)(), UniformRand(20, 100)());
			o->setCylindric(1, 1, 10);
			o->setColor(Color(0.9, 0.2, 0.2));   //radius, height, and mass
		...
		p2.push_back(Point(-5,-1));
		p2.push_back(Point(5,-1));
		for (int i = 0; i < 5; i++)             // 5 dark blue rectangular 
		{
			PhysicalObject* o = new PhysicalObject;
		...
		Marxbot *marxbot = new Marxbot;           //add a Marxbot
