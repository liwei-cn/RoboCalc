...
{
	double dist = sqrt(pow((GetXCoordinate() - object->GetXCoordinate()),2) + pow((GetYCoordinate() - object->GetYCoordinate()),2));

	std::cout << dist << " " << GetLeftSpeed() << " " << GetRightSpeed() << std::endl;
	if ((dist >= boundary - threshold) && (dist <= (boundary + threshold)))
	{
		SetLeftSpeed(0.5*EPuckMaximumSpeed);  //0.5
		SetRightSpeed(0.5*EPuckMaximumSpeed); //0.5
		myEnkiEpuck->setLedRing(0);
	}

	else if (dist < (boundary - threshold))
	{
		SetLeftSpeed(0.6*EPuckMaximumSpeed);  //0.6
		SetRightSpeed(0.4*EPuckMaximumSpeed); //0.4
		myEnkiEpuck->setLedRing(1);
	}

	else if (dist > (boundary + threshold))
	{
		SetLeftSpeed(0.4*EPuckMaximumSpeed);  //0.4
		SetRightSpeed(0.6*EPuckMaximumSpeed); //0.6
		myEnkiEpuck->setLedRing(1);
	}
