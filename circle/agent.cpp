...
void Agent::ObstacleAvoidance()
{
	...
	if ((myEnkiEpuck->leftSpeed + myEnkiEpuck->rightSpeed) < 0){         //move backward
	    if(Sy < 0) {
	...
	double sl = (1-alpha)*myEnkiEpuck->leftSpeed + dir*alpha*(EPuckMaximumSpeed);
	double sr = (1-alpha)*myEnkiEpuck->rightSpeed + dir*alpha*(-EPuckMaximumSpeed);
	SetLeftSpeed(sl);
	SetRightSpeed(sr);
